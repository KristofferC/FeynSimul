...
class RunParams:
    ...
    Variables that always need to be defined in the class before a run are:
      ...
      determines how many changes should be made to the path before the
      operator should be calculated along the whole path.
    ...
    corresponding parameters.
      ...
      - B{enablePathShift} (bool) E{-} Takes an interval of the path and
      offsets the whole interval.
      ...
      - B{enableSingleNodeMove} (bool) E{-} Offsets a single node in the path.
        - B{alpha} (float) E{-} The offset of the node of the path will be a
        random number between E{-} B{alpha} and B{alpha}.
    ...
    Variables that affect how the probability density is calculated are
      - B{xmin} (float) & B{xmax} (float) E{-} A hypercube with
      the sides B{xmin} to B{xmax} where the probability density
      will be calculated inside.
      - B{binResolutionPerDOF} E{-} Determines how many discretization points
      to do in a degree of freedom. The total number of bins will be
      binResolutionPerDOF to the power of the degrees of freedom in the system.

    You can chose what type of memory the paths that the threads are working on
    should be stored with B{enableGlobalPath} (bool). With enableGlobalPath
    they are stored in the global memory and else in the local memory.

    If B{returnPaths} (bool) is enabled the paths are fetched from the GPU
    after a run.
    """
    def __init__(self, enableBins=False, returnBinCounts=False,
                 enableOperator=True, enableCorrelator=False,
    ...
    def getMetroStepsPerRun(self):
        ...
        if self.enableParallelizePath:
            ...
            return (self.nbrOfWalkers * nbrOfThreadsPerWalker *
                    self.metroStepsPerOperatorRun * self.operatorRuns
                    * self.enableBisection)
        else:
            return (self.nbrOfWalkers * self.metroStepsPerOperatorRun *
                    self.operatorRuns * (self.enableBisection +
                        self.enablePathShift + self.enableSingleNodeMove))
